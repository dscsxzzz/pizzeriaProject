    ...
    <Wrapper @click.stop="$router.push('/login')">
      <div @click.stop class="formContainer">
        ...
        <transition name="loading-fade" appear>
          <div v-if="store.logging" class="lds-ellipsis">
            <div></div>
            <div></div>
            <div></div>
            <div></div>
  ...
  methods: {
    async tryGetCode() {
      const response = await fetch(
        `${baseUrl.baseUrl}/auth/forgot-password/${this.username}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 200) {
        (this.message = result.message), (store.user.id = result.id);
      } else this.message = "Something went wrong, try one more time";
    },
    async tryConfirmCode() {
      const response = await fetch(
        `${baseUrl.baseUrl}/auth/forgot-password-code/${this.code}`,
        {
          method: "POST",
          headers: {
            Accept: "application/json",
